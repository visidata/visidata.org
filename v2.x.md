...
As suggested by @jsvine, unfurl is a row-wise expansion of a column with containers.
   ...
   - `unfurl-col` is kind of like a shortcut for an expand followed by a melt
...
The original unfurl code included a
@jsvbinWhile integrating unfurl, 
and in fact there was a version in the rownum plugin, and another version in @jsvine's unfurl plugin, and other people have 6 y
...
that `i` is has remained one of the few unbound keystrokes, I decided to develop this set of increment concepts, and bind them to the `i` family of commands.
...
Which should you use for in the `fmtstr` attribute on the Columns sheet?  A [Twitter poll] was evenly split, and @tsibley suggested ¿porque no los dos?]()
