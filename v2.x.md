...
The roundtrip is a core design feature of VisiData.

```
vd -b input.json -o output.json
```

should have a minimal (or no) diff. For a while, this was not the case with jsons. The columns would get shuffled upon save.
...
Since the jsonl format permits the inclusion of tabs and newlines within cells, this will provide space to these formats to further mature.
